 ...
 -5.47972522e-17 , 1.01089942e+01,  5.50955464e-17, -1.01089942e+01]), np.array([0, 0, 0])]
    ...
    def calc_reward(self, done_mask, success):
        ...
        if old_inter_dist > self.inter_dist:
            reward += np.min([2.3-self.inter_dist, 1])
...
class WALK_ENV():
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            self.xml_path = "C:/kisang/Ant_control/rl_env/box_walk.xml"
		#C:/kisang/Ant_control/result_macc
		#C:/Users/gpu/kisang/Ant_control/rl_env/ant_box.xml
 ...
 -5.47972522e-17 , 1.01089942e+01,  5.50955464e-17, -1.01089942e+01]), np.array([0, 0, 0])]
    ...
    def check_done(self):
        ...
        #3. success = box in region
        elif ant_dist < minimum_dist:
            done_mask = 1
            success = 1
    ...
    def calc_reward(self, done_mask, success):
        ...
        old_dist = self.dist
        ant_pos = self.data.xpos[self.torso_id][0:2]
        self.dist = calc_distance(ant_pos, target_position)

        ant_velocity = np.zeros(6)
        mujoco.mj_objectVelocity(self.model,self.data, mujoco.mjtObj.mjOBJ_BODY, self.torso_id, ant_velocity, 0)

        
        #1. ant reward (default values... velocity and distance)
        if old_dist > self.dist: 
            reward = (6 - self.dist)*5
            #reward = np.exp((10 - dist)/2) # 15
        else:
            reward = 0
        ...
        #4. qvel
        if np.any(abs(self.data.qvel[7:]) < 0.001): #problem... box included
            reward -= 3
        #else:
        #    reward += 5
        if np.all(abs(self.data.qvel[7:]) < 0.001):
            reward -= 10
